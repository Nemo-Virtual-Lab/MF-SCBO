...
optimizer.optimize(max_cost=4000, batch_size=8, path="results/Ackley", verbose=True)

# Load results
data = np.load("results/Ackley/MFSCBO_data.npz", allow_pickle=True)
gps_info = data["gps_info"]
iterations = data["iterations"]
xvalues = data["cost_iter"]
bestY = data["best_Y"]
nbiters = data["nb_iter"]
print(sum(nbiters[0]),sum(nbiters[1]))
...
plt.tight_layout()
plt.savefig("results/Ackley/MFSCBO_plots.pdf")
plt.show()