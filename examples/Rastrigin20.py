from mfscbo.functions import Rastrigin
...
dim = 6
f0_ = Rastrigin(dim=dim, fidelity=1, fidelity_high=3, torchargs=torchargs)
f1_ = Rastrigin(dim=dim, fidelity=2, fidelity_high=3, torchargs=torchargs)
f2_ = Rastrigin(dim=dim, fidelity=3, fidelity_high=3, torchargs=torchargs)
...
f0 = lambda x : -eval_fun_unnormalized(x, f0_, bounds)[:, None]
f1 = lambda x : -eval_fun_unnormalized(x, f1_, bounds)[:, None]
f2 = lambda x : -eval_fun_unnormalized(x, f2_, bounds)[:, None]
eval_objectives = [f0, f1, f2]

# Define constraints
r_constraint = 2.0
center_constraint = torch.zeros(dim, **torchargs)
...
n_pts = [15,5,4]#[60, 15, 5] #total 4*dim = 60 + 15 + 5 = 80
rhos = [1, 1, 1]
costs = [1, 5, 20]
...
optimizer.optimize(max_cost=4000, batch_size=8, path="results/Rastrigin20", verbose=True)

# Load results
data = np.load("results/Rastrigin20/MFSCBO_data.npz", allow_pickle=True)
...
plt.tight_layout()
plt.savefig("results/Rastrigin20/MFSCBO_plots.pdf")
plt.show()